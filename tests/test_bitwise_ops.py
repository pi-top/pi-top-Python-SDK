...
@pytest.mark.parametrize(
    "data,no_of_bytes,expected_list",
    [
        [18, -1, [18]],
        [18, 5, [0, 0, 0, 0, 18]],
        [256, 5, [0, 0, 0, 1, 0]],
...
def test_join_bytes(byte_list, expected_value):
...
@pytest.mark.parametrize(
    "bitwise_data,num_of_bytes,expected_value",
    [
        [0x1010, -1, 0xFEF],
        [0x14, -1, 0xB],
        [0x00001, -1, 0x0],
...
def test_flip_bits(bitwise_data, num_of_bytes, expected_value):
...
@pytest.mark.parametrize(
    "bitmask_to_get,bitwise_data,expected_value",
    [
        [0x110, 0x1110, 0x110],
...
def test_get_bits(bitmask_to_get, bitwise_data, expected_value):
...
@pytest.mark.parametrize(
    "bitmask_to_ignore,bitwise_data,expected_value",
    [
        [0x100, 0x1110, 0x1010],
...
def test_ignore_bits(bitmask_to_ignore, bitwise_data, expected_value):
...
@pytest.mark.parametrize(
    "bits_to_raise,bitwise_data,expected_value",
    [
        [0x100, 0x1010, 0x1110],
        [0x01110001, 0x01110001, 0x01110001],
...
def test_set_bits_high(bits_to_raise, bitwise_data, expected_value):
...
@pytest.mark.parametrize(
    "bitmask_to_check,bitwise_data,expected_bool",
    [
        [0x100, 0x100, True],
        [0x100, 0x010, False],
...
def test_bitmask_is_on(bitmask_to_check, bitwise_data, expected_bool):
    test_value = bitwise.bitmask_is_on(bitmask_to_check, bitwise_data)
    assert expected_bool == test_value
...
@pytest.mark.parametrize(
    "bits_to_check,bitwise_data,expected_bool",
    [
...
def test_all_bits_are_on(bits_to_check: list, bitwise_data: int, expected_bool):
    test_list = bitwise.all_bits_are_on(bits_to_check, bitwise_data)
    assert expected_bool == test_list
