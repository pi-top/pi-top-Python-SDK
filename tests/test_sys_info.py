...
def test_network_interfaces_enum():
...
def test_is_pi_architecture_verification(uname_mock):
...
def test_debian_version_checks_version_file(open_mock, path_mock):
...
@patch("pitop.common.sys_info.path")
def test_debian_version_checks_if_version_file_exists(path_mock):
...
@patch("pitop.common.sys_info.path")
@patch("pitop.common.sys_info.open", new_callable=mock_open(), read_data="bullseye/sid")
def test_debian_version_returns_file_content(open_mock, path_mock):
    path_mock.exists.return_value = True
    from pitop.common.sys_info import get_debian_version
...
@patch("pitop.common.sys_info.path")
@patch("pitop.common.sys_info.open", new_callable=mock_open, read_data="VERSION_ID=99")
def test_debian_maj_version_checks_os_release_file(open_mock, path_mock):
...
@patch("pitop.common.sys_info.path")
@patch("pitop.common.sys_info.open", new_callable=mock_open, read_data="VERSION_ID=99")
def test_debian_maj_version_content(open_mock, path_mock):
    path_mock.exists = MagicMock(return_value=True)
    from pitop.common.sys_info import get_maj_debian_version
...
@patch("pitop.common.sys_info.path")
def test_debian_maj_version_checks_if_version_file_exists(path_mock):
...
def test_network_strength_runs_iwconfig(check_output_mock):
...
def test_network_strength_on_check_output_error(_):
    from pitop.common.sys_info import get_network_strength
...
@patch("pitop.common.sys_info.check_output")
def test_network_strength_parse_iwconfig(check_output_mock):
...
@patch("pitop.common.sys_info.check_output")
def test_network_strength_parse_iwconfig_without_expected_response(check_output_mock):
...
@patch("pitop.common.sys_info.check_output")
def test_wifi_network_ssid_runs_iwgetid(check_output_mock):
...
@patch("pitop.common.sys_info.check_output", side_effect=FileNotFoundError)
def test_wifi_network_ssid_on_error(_):
    from pitop.common.sys_info import get_wifi_network_ssid

    assert get_wifi_network_ssid() == "Error"


def test_internal_ip_on_unknown_interface():
...
def test_internal_ip_on_interface_on_ifaddresses_error(netifaces_mock):
...
@patch("pitop.common.sys_info.netifaces")
def test_internal_ip_on_interface_with_invalid_ifaddresses_response(netifaces_mock):
    netifaces_mock.interfaces.return_value = "wlan0"
...
@patch("pitop.common.sys_info.netifaces")
def test_internal_ip_on_interface_without_ip(netifaces_mock):
...
def test_start_systemd_service_command(run_command_mock):
...
@patch("pitop.common.sys_info.run_command")
def test_stop_systemd_service_command(run_command_mock):
...
@patch("pitop.common.sys_info.run_command")
def test_systemd_active_state_command(run_command_mock):
...
def test_systemd_active_state_response_on_error(run_command_mock):
    from pitop.common.sys_info import get_systemd_active_state
...
@patch("pitop.common.sys_info.check_output")
def test_systemd_enabled_state_command(check_output_mock):
...
def test_systemd_enabled_state_response_on_error(check_output_mock):
    from pitop.common.sys_info import get_systemd_enabled_state
...
def test_get_systemd_enabled_state_based_functions(get_systemd_enabled_state_mock):
...
@patch("pitop.common.sys_info.path")
@patch("pitop.common.sys_info.open", new_callable=mock_open())
def test_interface_is_up_checks_version_file(open_mock, path_mock):
...
@patch("pitop.common.sys_info.path")
def test_interface_is_up_checks_if_version_file_exists(path_mock):
    path_mock.exists.return_value = False
    from pitop.common.sys_info import interface_is_up
...
def test_interface_is_up_returns_file_content(open_mock, path_mock):
    path_mock.exists.return_value = True
    from pitop.common.sys_info import interface_is_up
...
@patch("pitop.common.sys_info.run_command")
def test_pings_to_determine_internet_connection_status(run_command_mock):
...
def test_get_address_for_connected_device_checks_dhcp_leases_if_file_exists(
    ...
    with patch.object(Path, "exists") as e:
        ...
        get_address_for_connected_device()
        isc_dhcp_leases_mock.assert_not_called()

    with patch.object(Path, "exists") as e:
        e.return_value = True
        get_address_for_connected_device()
        assert isc_dhcp_leases_mock.call_count > 0
...
def test_get_pi_top_ip_if_wlan0_has_ip(
    get_internal_ip_mock, get_address_for_connected_device_mock
):
    ip = "192.168.100.44"
...
def test_get_pi_top_ip_fallback_response(
    get_internal_ip_mock, InterfaceNetworkDataMock
):
    get_internal_ip_mock.side_effect = ValueError
