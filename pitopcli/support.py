...
class SupportCLI(CliBaseClass):
    ...
    def add_parser_arguments(cls, parser) -> None:
        subparser = parser.add_subparsers(
            title="pi-top support", description=cls.parser_help, dest="help_subcommand"
        )

        # pi-top support links
        links_parser = subparser.add_parser(
            "links", help="Find links to pi-top support pages"
        )

        # pi-top support links docs
        docs_subparser = links_parser.add_subparsers(
            title="Documentation",
        ...
        subparser.add_parser(
            "health_check", help="Perform a system verification to find possible issues"
        )
