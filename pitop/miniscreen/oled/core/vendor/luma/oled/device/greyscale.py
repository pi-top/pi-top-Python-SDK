...
class greyscale_device(device, __framebuffer_mixin):
    ...
    def _supported_dimensions(self):
        """Enumerates the screen resolutions that the specific device supports,
        as a list of tuples; e.g.: ``[(96, 64), (96, 32), (96, 16)]``"""
    ...
    def _init_sequence(self):
        ...
        Invoked from the constructor, but no assumptions should be made
        about what has been initialized so far. No return value is
        expected.
        """
        pass  # pragma: no cover
    ...
    def _set_position(self, top, right, bottom, left):
        """Invoked once as part of the devices display refresh.

        The four coordinates form a bounding box that determines the
        area of the screen that will get get redrawn; thus the concrete
        implementations should send the correct command sequence to the
        device to set that bounding box. No return value is expected.
        """
        pass  # pragma: no cover
    ...
    def _inflate_bbox(self, bounding_box):
        """Realign the left and right edges of the bounding box such that they
        are inflated to align modulo 4.

        this method is optional, and used mainly to accommodate devices
        with COM/SEG GDDRAM structures that store pixels in 4-bit
        nibbles.
        """
    ...
    def display(self, image):
        ...
        greyscale OLED display. RGB pixels are converted to 4-bit greyscale
        values using a simplified Luma calculation, based on.
