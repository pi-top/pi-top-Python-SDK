...
def get_bits(bitmask_to_get: int, bitwise_data: int):
    """
    This function will return the integer value represented by the bits that are high in
     `bitmask_to_get` from the bits that represent the integer value, `bitwise_data`.
...
def to_bits(int_to_convert: int):
    """
    This function will convert a integer into a binary string without a prefix (e.g. '0b');
...
def from_bits(bin_str: str):
    """
    This function will convert a binary string (with or without prefix) into a integer;
...
def pad_bits(bin_str: str, no_of_bytes: int):
    """
    This function will 'left zero pad' `bin_str`, if necessary,
     to ensure that it matches length `num_of_bytes`;
...
def ignore_bits(bitmask_to_ignore: int, bitwise_data: int):
    """
    This function will set the bits high in `bitmask_to_ignore` low in `bitwise_data`;
...
def set_bits_low(bits_to_set_low: int, bitwise_data: int):
    """
    This function will set bits low based on `bits_to_set_low` in `bitwise_data`;
...
def bitmask_is_on(bitmask_to_check: int, bitwise_data: int):
    """
    This function will check if bits are high in `bitwise_data` against bits that are high
    in `bitmask_to_check` and return True or False based on that;
...
def all_bits_are_on(bits_to_check: list, bitwise_data: int):
    """
    This function will determine if particular bits are high in some bitwise data
