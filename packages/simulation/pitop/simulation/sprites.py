...
def sprite_factory(name: str):
    import pygame

    class PitopSprite(pygame.sprite.Sprite, ComponentableSimSprite):
    ...
    class LEDSprite(pygame.sprite.Sprite, SimSprite):
    ...
    class ButtonSprite(pygame.sprite.Sprite, SimSprite):
        def __init__(self, config, scale, draw_port=False):
            pygame.sprite.Sprite.__init__(self)
    ...
    class BuzzerSprite(pygame.sprite.Sprite, SimSprite):
        ...
        def __init__(self, config, scale, draw_port=False):
            ...
            self.buzzer_sound_image = SimSprite._load_image(
                Images.buzzer_sound_icon, scale
            )
    ...
    class SliderSensorSpriteClass(pygame.sprite.Sprite, SimSprite):
        slider: Slider
        font: Component
        icon: Component
        img: Component
        ...
        def create_components(self):
            ...
            right_hand_side_x_start = (
                self.Size[0] - SLIDER_WIDTH - SLIDER_HANDLE_RADIUS - 1
            )
    ...
    class LightSensor(SliderSensorSpriteClass):
    ...
    class SoundSensor(SliderSensorSpriteClass):
        Size = (PMA_CUBE_SIZE[0] * 2 + MARGIN, PMA_CUBE_SIZE[1])
    ...
    class Potentiometer(SliderSensorSpriteClass):
        Size = (PMA_CUBE_SIZE[0] * 2 + MARGIN, PMA_CUBE_SIZE[1])
        image = Images.Potentiometer
        measurement_key_in_state = "position"
        icon = Images.angle_icon

    class UltrasonicSensor(SliderSensorSpriteClass):
    ...
    if name == "Pitop":
        ...
        return BuzzerSprite
    elif name == "SliderSensor":
        return SliderSensorSpriteClass
    ...
    elif name == "UltrasonicSensor":
        return UltrasonicSensor
    else:
        raise ValueError(f"Invalid sprite name: {name}")


class LazySpriteClass:
    """Provide class attribute access while maintaining lazy loading pygame"""
    ...
    def _get_class(self):
        """Lazily load the actual sprite class."""
        if self._class is None:
            self._class = sprite_factory(self._name)
        return self._class

    def __call__(self, *args, **kwargs):
        """Allow instantiation like the original partial function."""
        return self._get_class()(*args, **kwargs)

    def __getattr__(self, name):
        """Delegate attributes to the actual class, triggering lazy loading."""
        return getattr(self._get_class(), name)


Pitop = LazySpriteClass("Pitop")
LED = LazySpriteClass("LED")
Button = LazySpriteClass("Button")
Buzzer = LazySpriteClass("Buzzer")
SliderSensor = LazySpriteClass("SliderSensor")
LightSensor = LazySpriteClass("LightSensor")
SoundSensor = LazySpriteClass("SoundSensor")
Potentiometer = LazySpriteClass("Potentiometer")
UltrasonicSensor = LazySpriteClass("UltrasonicSensor")
